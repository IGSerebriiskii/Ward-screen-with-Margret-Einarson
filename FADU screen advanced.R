...
fadu_data2n_calc = fadu_data2n_calc %>% mutate(fr_change_sd = fr_change*sqrt((veh_sd/veh)^2 + (cbd_sd/cbd)^2), rep_count = 3) 
fadu_data2n_calc %>% head()          
...
fadu2n_controls_veh_mean = fadu_data2n_calc %>% filter(Well %in% fadu_controls) %>% select(5:7) %>%  unlist() %>% mean()
fadu2n_controls_veh_sd = fadu_data2n_calc %>% filter(Well %in% fadu_controls) %>% select(5:7) %>%  unlist() %>% sd()
fadu2n_controls_cbd_mean = fadu_data2n_calc %>% filter(Well %in% fadu_controls) %>% select(2:4) %>%  unlist() %>% mean()
fadu2n_controls_cbd_sd = fadu_data2n_calc %>% filter(Well %in% fadu_controls) %>% select(2:4) %>%  unlist() %>% sd()
fadu2n_controls_cbd_mean/fadu2n_controls_veh_mean  # about the same as fadu2n_controls_mean

fadu_data2n_calc %>% filter(Well %in% fadu_controls) %>% select(5:7) %>% unlist() %>%  mean()
# fadu_data_calc %>% filter(Well %in% fadu_controls) %>% pull(veh) %>% mean()
fadu2n_controls_mean # 0.5182667
fadu2n_controls_sd # .04414386

# combining data for controls (DMSO) wells
fadu2n_controls_calc = tibble(Well = "DMSO", 
                              CBD_pl_1 = NA, CBD_pl_2 = NA,CBD_pl_3 = NA, 
                              V_pl_1 = NA, V_pl_2 = NA, V_pl_3 = NA,
                              veh = fadu2n_controls_veh_mean, cbd = fadu2n_controls_cbd_mean, fr_change = cbd/veh,
                              log_fr_change = round(log2(fr_change),3),
                              veh_sd = fadu2n_controls_veh_sd, cbd_sd = fadu2n_controls_cbd_sd,
                              fr_change_sd = fr_change*sqrt((veh_sd/veh)^2 + (cbd_sd/cbd)^2), rep_count = length(fadu_controls))


fadu_data2n_calc_for_plot_gem %>% mutate(fr_change = cbd/veh) %>% pull(fr_change) %>% mean()
# 0.9018924
fadu_data2n_calc_for_plot_gem %>% mutate(fr_change = cbd/veh) %>% pull(fr_change) %>% sd()
# 0.06066369
fadu_data2n_calc_for_plot_doc %>% mutate(fr_change = cbd/veh) %>% pull(fr_change) %>% mean()
# 0.7843344
fadu_data2n_calc_for_plot_doc %>% mutate(fr_change = cbd/veh) %>% pull(fr_change) %>% sd()
# 0.08396704
...
fadu_hits2nc = bind_rows(fadu_hits2n,fadu2n_controls_calc )



fadu_hits2nc = fadu_hits2nc %>% mutate(p_val_diff = 1)
for (i in 1:41) {
          fadu_hits2nc$p_val_diff[i] =  tsum.test(mean.x=fadu_hits2nc$fr_change[i],   s.x=fadu_hits2nc$fr_change_sd[i], n.x=fadu_hits2nc$rep_count[i],
                                       mean.y=fadu_hits2nc$fr_change[42],   s.y=fadu_hits2nc$fr_change_sd[42], n.y=fadu_hits2nc$rep_count[42])$p.value
          
}

fadu_hits2nc %>% write_csv("fadu_hits2_nuclei.csv")

fadu_data2n_calc_for_plot_comp = fadu_data2n_calc %>% filter(Well %in% fadu_inner_compounds) %>% 
          select(veh,cbd,cbd_sd, veh_sd ) %>% mutate(well = "compounds")
fadu_data2n_calc_for_plot_control = fadu_data2n_calc %>% filter(Well %in% fadu_controls) %>% 
          select(veh,cbd,cbd_sd, veh_sd) %>% mutate(well = "controls")
fadu_data2n_calc_for_plot_gem = fadu_data2n_calc %>% filter(Well %in% fadu_gem) %>% 
          select(veh,cbd,cbd_sd, veh_sd) %>% mutate(well = "gem")
fadu_data2n_calc_for_plot_doc = fadu_data2n_calc %>% filter(Well %in% fadu_doc) %>% 
          select(veh,cbd,cbd_sd, veh_sd) %>% mutate(well = "doc")
...
fadu_data2n_calc_for_plot  %>% 
          ...
          geom_point(data = fadu_hits2n, aes(x = veh, y = cbd, color = "hits"),color = "black") +
          coord_cartesian(xlim = c(3000,15000))
ggsave("veh vs cbd run2 nuclei detailed with errorbars hits.png")
...
fadu_data2n_calc_for_plot  %>% 
          ...
          geom_point(data = fadu_hits2n, aes(x = veh, y = cbd, color = "hits"),color = "black") +
          coord_cartesian(xlim = c(3000,15000))
ggsave("veh vs cbd run2 nuclei non-linear with errorbars hits.png")
fadu_data2n_calc_for_plot_gem %>% mutate(fr_change = cbd/veh) %>% pull(fr_change) %>% mean()
# 0.9018924
...
# -------------------
# ----overall correllation between cbd and vehicle - to be used for uncertainty propagation

fadu_data2_calc %>%  select(veh,cbd) %>% cor() # 0.948207 shows 2 x 2 table, pairwise correllation
cor(fadu_data2_calc$veh, fadu_data2_calc$cbd) #interestingly, shows single value
fadu_data2n_calc %>%  select(veh,cbd) %>% cor() # 0.8555105

fadu_hits2 = fadu_hits2 %>% 
fadu_hits2n = fadu_hits2n %>% 